		...
		ImGui::SetColumnOffset(1, 120);
		{
			...
			ImGui::NextColumn();
			{
				...
				if (Tab == 1)
				{
					...
					{
						ImGui::Spacing();
						ImGui::Checkbox("Display Fov", &isEspfovshow);
						if (isEspfovshow)
						{
							ImGui::Spacing();
				...
				if (Tab == 2)
				{
					...
					ImGui::Spacing();
					ImGui::Spacing();
					ImGui::Checkbox("Display 2D Box", &isEspBox);
					ImGui::Spacing();
					ImGui::Checkbox("Display Snaplines", &isEspLines);
					ImGui::Spacing();
					ImGui::Checkbox("Display Head Circle", &isEspHead);
				}
				...
				if (Tab == 3)
				{
					...
					ImGui::Spacing();
					ImGui::Spacing();
					ImGui::Checkbox("Display FPS", &isEspFPS);
					ImGui::Spacing();
					ImGui::Checkbox("Display Watermark", &isEspwatermark);
					ImGui::Spacing();
					ImGui::Checkbox("Refresh Team Check", &isChangeTeam);
					//ImGui::Spacing();
					//ImGui::Checkbox("Enable Fast Overlay (may cause aimbot lag)", &fastoverlay);
				}
			}
		}
	}
...
void AimbotLoop()
{
	...
	for (int i = 0; i < Entity.size(); i++)
	{
		...
		if (isAimbot && closestPawn && GetAsyncKeyState(KeyCodes(SelectedKey)))
		{
